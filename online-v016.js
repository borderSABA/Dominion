...
function setOnlineGameUi(playing){
  ...
  const leave=document.getElementById("leaveGameOnline"),conn=document.getElementById("onlineGameConn");
...
buildSupply=function(){const n=Math.max(2,Math.min(6,state.playerCount||totalLobbyPlayers()||2));const pool=[...state.selectedSets].flatMap(s=>KINGDOM_BY_SET[s]||[]),uniq=[...new Set(pool)];const fixed=[...new Set((state.customKingdom||[]).filter(x=>CARDS[x]))].slice(0,10),remaining=uniq.filter(x=>!fixed.includes(x));if(fixed.length<10&&remaining.length<10-fixed.length){toast("王国カード候補が10山に足りません");return false;}state.kingdom=[...fixed,...shuffle([...remaining]).slice(0,10-fixed.length)];const vp=victoryPileSize(n);state.supply={"銅貨":copperSupply(n),"銀貨":n>=5?80:40,"金貨":n>=5?60:30,"屋敷":vp,"公領":vp,"属州":provinceCount(n),"呪い":curseCount(n)};if(state.selectedSets.has("錬金術")){if(!CARDS["ポーション"])add(C("ポーション",4,["財宝"],"錬金術","+1ポーション","generic",null,{treasurePotions:1}));state.supply["ポーション"]=16;}if(state.selectedSets.has("繁栄")){if(!CARDS["白金貨"])add(C("白金貨",9,["財宝"],"繁栄","+5コイン","generic",null,{treasureCoins:5}));if(!CARDS["植民地"])add(C("植民地",11,["勝利点"],"繁栄","+10勝利点","generic",()=>10,{}));state.supply["白金貨"]=n>=5?24:12;state.supply["植民地"]=provinceCount(n);}if(state.selectedSets.has("移動動物園")){if(!CARDS["馬"])add(C("馬",3,["アクション"],"移動動物園","+2カード｜+1アクション｜使用後、馬の山へ戻る","generic",null,{draw:2,actions:1,returnToSupply:true}));state.supply["馬"]=30;}if(state.selectedSets.has("暗黒時代")){if(!CARDS["廃墟"])add(C("廃墟",0,["アクション","廃墟"],"暗黒時代","廃墟の混成山札","generic"));state.supply["廃墟"]=10*(n-1);}for(const x of state.kingdom)state.supply[x]=typeHas(x,"勝利点")?vp:10;renderSupplyPreview();return true;};
...
const coreRender=render;render=function(){coreRender();renderMultiPlayers();renderSharedPlayArea();if(!NET.suppress&&NET.connected&&state.inGame){const active=state.players[state.turn];if(localIsActive()||(NET.isHost&&active?.isCPU))schedulePush();}};
...
function createParticipants(){const hs=connectedHumans().map(m=>({name:m.name,isCPU:false,netSeat:m.seat}));let used=new Set(hs.map(x=>x.netSeat)),cpus=[];for(let i=0;i<NET.cpuCount;i++){let seat=0;while(used.has(seat))seat++;used.add(seat);cpus.push({name:`CPU ${i+1}`,isCPU:true,netSeat:null});}let all=shuffle([...hs,...cpus]);all.forEach((p,i)=>p.gameSeat=i);return all;}
...
function startOnlineGame(){if(!NET.isHost)return;const parts=createParticipants();if(parts.length<2||parts.length>6)return toast("2～6人で開始してください");state.playerCount=parts.length;if(!buildSupply())return;setupPlayersMulti(parts);state.trash=[];state.logs=[];state.inGame=true;state.turn=Math.floor(Math.random()*state.players.length);showGame();log(`<b>ゲーム開始</b>：${state.players[state.turn].name} が先手です。`);startTurn();send({type:"start",snapshot:canonicalSnapshot()});pushState(true);maybeRunCpu();}
...
async function cpuTurnMulti(){
  ...
  if(pick){
    const ok=buyCard(p,pick);await sleep(900);
    state.busy=false;
    if(ok&&state.inGame&&state.players[state.turn]===p){endCurrentTurn();}
    return;
  }
  state.busy=false;endCurrentTurn();
}
...
async function attackEach(actor,fn){for(const target of opponentsOf(actor)){if(attackBlocked(target))continue;await fn(target);}}
...
banditAttack=async function(p){await attackEach(p,async o=>{const before=score(o),revealed=[];for(let i=0;i<2;i++){const n=takeTop(o);if(n)revealed.push(n);}const targets=revealed.filter(n=>typeHas(n,"財宝")&&n!=="銅貨");let trashName=null;if(targets.length===1)trashName=targets[0];else if(targets.length>1){if(o.isCPU)trashName=[...targets].sort((a,b)=>CARDS[b].cost-CARDS[a].cost)[0];else if(isLocalHuman(o))trashName=await cardNameModal("山賊の攻撃","廃棄する財宝を1枚選択",targets);else trashName=await remoteChoice(o,"name",{title:"山賊の攻撃",desc:"廃棄する財宝を1枚選択",names:targets});}let used=false;for(const n of revealed){if(!used&&n===trashName){state.trash.push(n);o.trash.push(n);used=true;log(`${o.name} の <b>${n}</b> が山賊で廃棄`);}else o.discard.push(n);}const after=score(o);if(after!==before)showPointPop(o,after-before);});};
const coreResolveEffect=resolveEffect;resolveEffect=async function(p,name){if(name==="議事堂"){draw(p,4);state.buys++;for(const o of opponentsOf(p))draw(o,1);render();return;}if(name==="魔女"){draw(p,2);await attackEach(p,async o=>gain(o,"呪い"));render();return;}await coreResolveEffect(p,name);};
...
buyCard=function(p,name){
  ...
  if(!p.isCPU){
    setTimeout(()=>{
      if(state.inGame&&state.players[state.turn]===p&&!state.busy)endCurrentTurn();
    },120);
  }
  return true;
};
