...
function showTitle(){document.getElementById("roomScreen").classList.remove("active");document.getElementById("titleScreen").classList.add("active");document.getElementById("roomTabs").style.display="none";setOnlineGameUi(false);resetLocalRoomState();NET.name=readCommonName()||NET.name;renderTitle();}
...
async function joinRoom(room,opt={}){
  const inputName=(document.getElementById("onlineName")?.value||NET.name||opt.name||"プレイヤー").trim().slice(0,16)||"プレイヤー";
  NET.name=inputName;
  const roomInfo=(NET.roomsCache||[]).find(x=>Number(x.room)===Number(room));
  const humans=roomInfo?.humanCount??roomInfo?.connectedHumans??0;
  const hasMembers=(roomInfo?.memberCount??humans)>0 || !!roomInfo?.started;
  if(!opt.resume&&!hasMembers){
    const msg=document.getElementById("titleServerMsg");
    if(msg)msg.textContent="ホスト権限を確認中…";
    try{await checkHostAuthority(inputName,room);}
    catch(e){if(msg)msg.textContent=e.message||"ホスト権限がありません";return;}
    if(msg)msg.textContent="";
  }
  // ROOM参加だけでは共通プレイヤー名を保存しない。
  NET.room=room;NET.intentionalLeave=false;NET.resumeAttempt=!!opt.resume;
  ...
  ws.onopen=()=>send({type:"join",clientId:NET.clientId,name:inputName,resume:!!opt.resume,hostCheckName:inputName});
...
function canonicalSnapshot(){const plain={};for(const k of Object.keys(state)){if(k==="supplyPick"||k==="busy")continue;let v=state[k];if(v instanceof Set)v=[...v];else v=JSON.parse(JSON.stringify(v));plain[k]=v;}plain.busy=false;plain.supplyPick=null;const activeSeat=state.players[state.turn]?.gameSeat??0;plain.players=[...plain.players].sort((a,b)=>(a.gameSeat??0)-(b.gameSeat??0));
  plain.players.forEach(p=>{try{p.publicScore=score(p)}catch{p.publicScore=p.publicScore||0}});
  plain.turn=plain.players.findIndex(p=>p.gameSeat===activeSeat);return plain;}
...
function startOnlineGame(){
  ...
  state.gameInstanceId=`dominion-${NET.room}-${Date.now()}-${Math.random().toString(36).slice(2,8)}`;
  state.startedAt=new Date().toISOString();
  state.playRecordSubmitted=false;
  saveCommonNameOnGameStart(NET.name);
  ...
  send({type:"start",snapshot:snap,playRecord:{
    gameId:"dominion",
    gameName:"ドミニオン",
    roomNo:NET.room,
    roomId:`ROOM-${NET.room}`,
    gameInstanceId:state.gameInstanceId,
    startedAt:state.startedAt,
    players:snap.players.map(p=>p.name)
  }});
  submitPlayRecordOnce(snap);
  pushState(true);maybeRunCpu();
}
...
["commonHomeTop","commonHomeTitle"].forEach(id=>rebind(id,goCommonHome));
["commonRecordTop","commonRecordTitle"].forEach(id=>rebind(id,goCommonRecord));
